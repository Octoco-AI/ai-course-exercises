...
async def test_agent_dispatches_tool_then_finishes(settings_for_test, sandbox, monkeypatch):
    """Model calls list_tickets, then replies with final text on turn 2. Needs Step 4."""
    scripted = [
    ...
    try:
        result = await run_agent("what's open?", tools=sandbox["tools"], settings=settings_for_test)
...
async def test_agent_handles_tool_error_gracefully(settings_for_test, sandbox, monkeypatch):
    ...
    scripted = [
        [gtypes.Part(function_call=gtypes.FunctionCall(name="read_ticket", args={"ticket_id": "TKT-99999"}))],
        [gtypes.Part(text="Sorry, that ticket doesn't exist.")],
    ]
    monkeypatch.setattr("backend.agent.genai.Client", lambda api_key: _FakeGeminiClient(scripted))

    try:
        result = await run_agent("read TKT-99999", tools=sandbox["tools"], settings=settings_for_test)
    except NotImplementedError:
        pytest.skip("Tool dispatch not implemented yet — Module 11, Step 4.")
