"""Module 12, Step A.2 — SSE event-shape tests.

`backend/streaming.py` doesn't exist on a fresh Module 11 starter — that's
expected; the whole module is skipped until it's created.

Run just these with: pytest -m m12 tests/m12/test_streaming.py
"""
...
streaming = pytest.importorskip(
    "backend.streaming", reason="backend/streaming.py doesn't exist yet — Module 12, Step A.2."
)
