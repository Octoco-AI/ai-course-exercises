"""Module 12, Step A.3 — the /api/chat endpoint.

Skips at module level until `ChatRequest` (and the route it backs) exist on
`backend.server` — that's the expected state through Module 11.

Run just these with: pytest -m m12 tests/m12/test_api.py
"""
...
if not hasattr(server_module, "ChatRequest"):
    pytest.skip(
        "POST /api/chat not implemented yet — Module 12, Step A.3.",
