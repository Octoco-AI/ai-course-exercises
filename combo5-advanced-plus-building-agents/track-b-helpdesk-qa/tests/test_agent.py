...
# ---------------------------------------------------------------------------
# Gemini path (the default). Mocks backend.agent.genai.Client with a scripted
# stream built from real google-genai types, so the loop's reconstruction of
# Content/Part objects runs for real.
# ---------------------------------------------------------------------------
...
class _FakeGeminiStream:
    def __init__(self, parts: list):
        # One chunk carrying all parts for the turn.
        self._chunks = [
            SimpleNamespace(
                candidates=[SimpleNamespace(content=_gtypes.Content(role="model", parts=parts))]
            )
        ]
...
@pytest.mark.asyncio
async def test_gemini_agent_terminates_on_text(sandbox, gemini_settings_for_test, monkeypatch):
    """Gemini path: model returns text only → text events + a done."""
    monkeypatch.setattr(
        "backend.agent.genai.Client",
        lambda api_key: _FakeGeminiClient([[_gtypes.Part(text="Hello there")]]),
    )

    events = []
    async for event in run_agent_streaming(
        "hi", settings=gemini_settings_for_test, tools=sandbox["tools"]
    ):
        events.append(event)
...
@pytest.mark.asyncio
async def test_gemini_agent_dispatches_tool_then_finishes(
    sandbox, gemini_settings_for_test, monkeypatch
):
    """Gemini path: search_kb + create_draft_reply on turn 1-2, final text on turn 3."""
    scripted = [
        [_gtypes.Part(function_call=_gtypes.FunctionCall(name="search_kb", args={"query": "enable 2FA"}))],
        ...
        [_gtypes.Part(text="I've drafted a reply about enabling 2FA. A human will review and send.")],
    ]
    monkeypatch.setattr(
        "backend.agent.genai.Client",
        lambda api_key: _FakeGeminiClient(scripted),
    )

    events = []
    async for event in run_agent_streaming(
        "How do I enable 2FA?", settings=gemini_settings_for_test, tools=sandbox["tools"]
    ):
        events.append(event)
