...
class _FakeGeminiStream:
    def __init__(self, parts: list):
        # One chunk carrying all parts for the turn — real chunking is a
        # server-side streaming detail this fake doesn't need to model.
        self._chunks = [
            SimpleNamespace(
...
async def test_streaming_agent_dispatches_tool_then_finishes(settings_for_test, sandbox, monkeypatch):
    ...
    events = [
        event
        async for event in run_agent_streaming(
            "what's here?", settings=settings_for_test, tools=sandbox["tools"]
        )
    ]
