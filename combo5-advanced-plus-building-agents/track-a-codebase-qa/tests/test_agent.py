...
async def test_gemini_agent_dispatches_tool_then_finishes(
    ...
    """Gemini path: function_call on turn 1, final text on turn 2."""
    scripted = [
        [_gtypes.Part(function_call=_gtypes.FunctionCall(name="list_files", args={"path": "."}))],
        [_gtypes.Part(text="Here are the files.")],
    ]
    monkeypatch.setattr(
        "backend.agent.genai.Client",
    ...
    events = []
    async for event in run_agent_streaming(
        "what's here?", settings=gemini_settings_for_test, tools=sandbox["tools"]
    ):
        events.append(event)
