...
echo "=== Tiny Agent pre-flight check ==="
echo
...
if python3 -c 'import google.genai' 2>/dev/null; then
    GENAI_VERSION=$(python3 -c 'import google.genai; print(getattr(google.genai, "__version__", "unknown"))')
    pass "google-genai installed (version: $GENAI_VERSION)"
else
    fail "google-genai not installed. Run: pip install -e ."
fi
