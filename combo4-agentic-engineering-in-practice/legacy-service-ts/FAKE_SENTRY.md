...
Day 2." Each one is real: it maps to code in `src/`. Your job (Modules 8 and
11) is to trace each from the report to the line and confirm it.
...
```
Error: order 00000021 total does not reconcile: stored=89.95 recomputed=89.96 delta=0.01
    at money (/srv/orderbase/src/utils.ts:19)
    at computeTotal (/srv/orderbase/src/orders.ts:60)
    at reconcileDay (/srv/ops/reconcileTotals.ts:66)
```
...
- **Culprit:** `src/orders.ts in dailyReport`
...
```
Error: daily digest got 0 orders for 2026-06-30 (expected >= 8)
    at dailyReport (/srv/orderbase/src/orders.ts:245)
    at dailyReport (/srv/orderbase/src/orders.ts:239)
    at sendDigest (/srv/ops/dailyDigest.ts:40)
```
...
- **Culprit:** `src/orders.ts in getOrder`
...
```
Error: order 00000007 API status=NEW but DB status=SHIPPED (process uptime 3d)
    at getOrder (/srv/orderbase/src/orders.ts:166)
    at checkOrder (/srv/ops/consistencyProbe.ts:52)
```
