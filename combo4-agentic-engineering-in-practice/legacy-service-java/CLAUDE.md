# OrderBase (Java) — Engineering Guidelines & Working Notes
...
- Java only in this repo. Target Java 21.
...
- Prefer plain static classes over Spring-managed beans, unless the code
  genuinely needs a request-scoped seam. `Orders`, `Db`, `Utils` and
  `LoggingSetup` carry no Spring annotations on purpose.
- Do not add heavy dependencies. We are JDK-first here on purpose; the ops
  boxes only ship the golden-AMI runtime plus what's in this repo.
- Do NOT add Spring Data JPA, Hibernate, or any ORM. We build SQL and hand it
  to `sqlite-jdbc` via `PreparedStatement`. This is a hard rule.

## Our history

We started on Java 8 with a hand-rolled `com.sun.net.httpserver` listener in
2018 and moved to Spring Boot in 2022 purely for the HTTP layer — domain code
stayed untouched. We used to run the suite with a plain JUnit 4 `TestRunner`
main class; that is gone now. We also used to have a big CSV exporter and a
separate reporting service; both were retired, though some helper code from
them still lingers in `Utils.java` (e.g. `chunk`, `toCents`) — leave it unless
you are sure nothing calls it. We standardised on raw JDBC over an ORM in
2018 because of the golden-AMI constraint, and that decision still holds.

## Running and testing

- Run the service with: `./mvnw spring-boot:run` (listens on port 5057).
- The old way was `./mvnw spring-boot:run -Dspring-boot.run.fork=false` with a
  file-watcher wired in, but the reloader double-binds `:5057` — do not use
  it.
- Run the tests with `./mvnw test` from the repo root.
- There is a `make test` target in some of our other repos; there is NOT one
  here, so just call `./mvnw test` directly.
...
For reference, a successful `POST /orders` returns something shaped like
this:
...
- Run `./mvnw test` and make sure it is green.
...
repo. TODO: split `Utils.java` into smaller files. TODO: finish the migration
