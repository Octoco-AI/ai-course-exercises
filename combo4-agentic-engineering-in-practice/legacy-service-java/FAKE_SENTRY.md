...
Day 2." Each one is real: it maps to code in `src/main/java/ai/octoco/legacyservice/`.
Your job (Modules 8 and 11) is to trace each from the report to the line and
confirm it.
...
- **Culprit:** `ai.octoco.legacyservice.Utils.money`
...
LedgerMismatchException: order 00000021 total does not reconcile:
	...
	at ai.octoco.legacyservice.Utils.money(Utils.java:47)
	at ai.octoco.legacyservice.Orders.computeTotal(Orders.java:58)
	at ops.ReconcileTotals.reconcileDay(ReconcileTotals.java:66)
```
...
- **Culprit:** `ai.octoco.legacyservice.Orders.dailyReport`
...
ReportEmptyException: daily digest got 0 orders for 2026-06-30 (expected >= 8)
	...
	at ai.octoco.legacyservice.Orders.dailyReport(Orders.java:221)
	at ai.octoco.legacyservice.Orders.dailyReport(Orders.java:215)
	at ops.DailyDigest.sendDigest(DailyDigest.java:40)
```
...
- **Culprit:** `ai.octoco.legacyservice.Orders.getOrder`
...
StaleReadException: order 00000007 API status=NEW but DB status=SHIPPED (process uptime 3d)
	...
	at ai.octoco.legacyservice.Orders.getOrder(Orders.java:153)
	at ops.ConsistencyProbe.checkOrder(ConsistencyProbe.java:52)
```
