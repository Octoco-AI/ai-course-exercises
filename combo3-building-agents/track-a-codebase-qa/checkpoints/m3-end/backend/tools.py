"""The tools the agent can call. Module 2 end state.

Three tools: `read_file`, `list_files`, `edit_file`. `list_files`'s
description was rewritten with a usage example (Step 2), and `read_file`
raises a structured `ToolError` with a recovery suggestion (Step 4). See
NOTES.md for the before/after.
...
def read_file(path: str, *, workspace: Path) -> str:
    """Read a file inside the workspace. Raises ToolError with a
    suggestion instead of a bare exception."""
    try:
