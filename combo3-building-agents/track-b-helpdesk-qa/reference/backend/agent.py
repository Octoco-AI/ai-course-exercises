...
(an `AsyncGenerator[str, None]`). The Anthropic client itself is still the
*synchronous* client — wrapping the outer function as `async def ...
yield` is what lets FastAPI's `StreamingResponse` consume it.
"""
