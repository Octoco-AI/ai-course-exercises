...
Module 1 ships a blocking `run_agent(...) -> str`; this prints the final
answer. Module 3 renames it to `run_agent_streaming(...)` (an async
generator) — pass `--stream` to drive that instead.
...
def main() -> int:
    ...
    tools = build_toolset(settings.workspace_root, draft_replies=settings.draft_replies_root)
    ...
    if stream:
        ...
        if run_agent_streaming is None:
            print(
                "ERROR: run_agent_streaming isn't implemented yet — that's Module 3.",
                file=sys.stderr,
            )
            return 2
    ...
    if run_agent is None:
        print(
            "ERROR: run_agent isn't defined — did Module 3 already rename it to "
            "run_agent_streaming? Pass --stream.",
            file=sys.stderr,
        )
        return 2

    result = run_agent(prompt, tools=tools, settings=settings)  # sync — no asyncio.run
    print(result)
    return 0
