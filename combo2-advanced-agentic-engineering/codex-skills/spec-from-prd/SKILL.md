...
The PRD is whatever the user named or pasted in the message that invoked this skill — a path, or the feature name if the PRD isn't named yet.

1. If a path was given and it exists, read it.
2. If a bare name was given, or nothing was, look for a PRD before asking:
   - Check `./docs/`, `./`, `./product/`, and `./prd/` for a markdown file whose first line matches `# PRD:`.
...
0. `./specs/<feature-slug>.prd.md` present? → Phase 0 is done.
...
These are the same artefact paths `spec-driven` uses (minus the frozen PRD copy, which is new here). A feature specified with this skill can be resumed with the `spec-driven` skill and vice versa — the state detection in both reads the same files.
...
**Do:**
1. Freeze the PRD: copy it verbatim to `./specs/<feature-slug>.prd.md` (create `./specs/` if needed). Never edit this copy — the PM's own document may keep changing; this frozen copy is what the spec traces against.
...
**Do:**
1. Check for `./AGENTS.md`. If it has a `## Constitution` section, read it and skip to Phase 2.
2. If it doesn't exist, draft one from what you actually have, using `templates/constitution-template.md`:
...
5. On approval, write it to `./AGENTS.md`. If an `AGENTS.md` already exists at the project root, ask where the user wants the constitution section added — Codex reads the whole file, so ordering is a readability choice, not a functional one.
...
6. Write the spec to `./specs/<feature-slug>.md`.
...
4. Record all clarifications — the PRD's open questions and the checklist findings alike — in a `## Clarifications` section appended to `./specs/<feature-slug>.md`. Preserve the original spec text; don't overwrite.
...
4. Write the plan to `./plans/<feature-slug>.md`.
...
4. Write the task list to `./tasks/<feature-slug>.md`.
...
- `./specs/<feature-slug>.prd.md` — the frozen PRD this run started from
- `./AGENTS.md` with the project constitution
- `./specs/<feature-slug>.md` — spec, traceability, clarifications
