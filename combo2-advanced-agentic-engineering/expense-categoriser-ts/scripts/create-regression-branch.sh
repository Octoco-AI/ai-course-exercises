...
#   - Tweaks the system prompt in src/core.ts to bias the model toward "Other"
#     for edge cases. This tanks the accuracy metric enough to fail the eval gate.
#   - Commits the change.
#
# What it demonstrates:
#   - A PR opened against main from this branch will trigger evals.yml, which
#     will fail on ACCURACY_THRESHOLD, which will block the merge.
...
const before = `- "confidence" is your self-reported certainty. Use 0.9+ for obvious matches
  (grocery store -> Food & Dining), 0.5-0.7 for ambiguous cases, below 0.5
  for genuinely unclear items.`;

const after = `- "confidence" is your self-reported certainty. When in doubt, use low
...
if (regressed === original) {
  console.error("Prompt block not found — has core.ts changed since the regression script was written?");
  process.exit(1);
}

writeFileSync(path, regressed);
console.log("Regressed core.ts.");
JS
