...
Day 2." Each one is real: it maps to code in `src/LegacyService/`. Your job
...
- **Culprit:** `LegacyService.Utils.Money`
...
```
LedgerMismatchException: order 00000021 total does not reconcile:
    stored=89.95  recomputed=89.96  delta=0.01

   at LegacyService.Utils.Money(Double x) in /srv/orderbase/src/LegacyService/Utils.cs:line 22
   at LegacyService.Orders.ComputeTotal(List`1 items, Double discountPct) in /srv/orderbase/src/LegacyService/Orders.cs:line 72
   at Ops.ReconcileTotals.ReconcileDay(DateTime day) in /srv/ops/ReconcileTotals.cs:line 66
```
...
- **Culprit:** `LegacyService.Orders.DailyReport`
...
```
ReportEmptyException: daily digest got 0 orders for 2026-06-30 (expected >= 8)

   at LegacyService.Orders.DailyReport(String dateStr) in /srv/orderbase/src/LegacyService/Orders.cs:line 258
   at LegacyService.Orders.DailyReport(String dateStr) in /srv/orderbase/src/LegacyService/Orders.cs:line 251
   at Ops.DailyDigest.SendDigest() in /srv/ops/DailyDigest.cs:line 40
```
...
- **Culprit:** `LegacyService.Orders.GetOrder`
...
```
StaleReadException: order 00000007 API status=NEW but DB status=SHIPPED (process uptime 3d)

   at LegacyService.Orders.GetOrder(String orderId) in /srv/orderbase/src/LegacyService/Orders.cs:line 175
   at Ops.ConsistencyProbe.CheckOrder(String orderId) in /srv/ops/ConsistencyProbe.cs:line 52
```
