...
- The **fulfilment / WMS sync** runs at 05:30 UTC. It imports the warehouse
  CSV and calls `updateOrderStatus(...)` for each shipped order. It does not
...
# nodemon is off on purpose -- it double-binds :5057 under the ESM loader and
# the unit flaps
```
...
- The process logs an `ExperimentalWarning` line to stderr on boot, from the
  sqlite driver. Ignore it; ops filtered it out of the journal in 2024.
