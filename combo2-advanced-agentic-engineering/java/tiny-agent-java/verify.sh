...
echo "=== Tiny Agent (Java) pre-flight check ==="
echo
...
if [ "$FAILED" -eq 0 ]; then
    if ./mvnw -q install -DskipTests >/dev/null 2>&1; then
        pass "modules build and install"
    else
        fail "build failed. Run './mvnw install -DskipTests' to see why."
    fi
fi

# The reference implementation must pass its own tests.
if [ "$FAILED" -eq 0 ]; then
    if TINY_AGENT_IMPL=reference ./mvnw -q -pl tests -am test >/dev/null 2>&1; then
        pass "reference implementation passes all tests"
    else
        fail "reference tests failed — that shouldn't happen. Email workshops@octoco.ai with the output of: TINY_AGENT_IMPL=reference ./mvnw -pl tests -am test"
    fi
fi

# Your own tests are EXPECTED to fail before the workshop.
if [ "$FAILED" -eq 0 ]; then
    if ./mvnw -q -pl tests -am test >/dev/null 2>&1; then
