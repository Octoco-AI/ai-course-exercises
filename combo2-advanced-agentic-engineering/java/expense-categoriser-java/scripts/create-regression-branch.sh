...
#   - Tweaks the system prompt in
#     src/main/java/ai/octoco/expensecategoriser/Core.java to bias the model
#     toward "Other" for edge cases. This tanks the accuracy metric enough to
#     fail the eval gate.
#   - Commits the change.
#
...
path = Path("src/main/java/ai/octoco/expensecategoriser/Core.java")
...
if regressed == original:
    raise SystemExit("Prompt block not found -- has Core.java changed since the regression script was written?")

path.write_text(regressed)
print("Regressed Core.java.")
PY
