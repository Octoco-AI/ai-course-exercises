...
echo "=== Expense Categoriser (Java) pre-flight check ==="
echo
...
# Build
if [ "$FAILED" -eq 0 ]; then
    if ./mvnw -q compile >/dev/null 2>&1; then
        pass "service builds"
    else
        fail "build failed. Run './mvnw compile' to see why."
    fi
fi
...
if [ "$FAILED" -eq 0 ]; then
    TEST_LOG=$(mktemp)
    if ./mvnw -q test >"$TEST_LOG" 2>&1; then
        pass "unit + API tests pass (23 tests)"
    else
        fail "unit + API tests failed. Run: ./mvnw test"
        tail -30 "$TEST_LOG" | sed 's/^/    /'
    fi
    rm -f "$TEST_LOG"
fi

# API key — a warning, not a failure. The unit tests fake the LLM.
if [ -f .env ]; then
    set -a
...
if [ "$RUN_EVALS" -eq 1 ] && [ "$FAILED" -eq 0 ]; then
    ...
    if [ -z "${GOOGLE_API_KEY:-}" ] || [ "${GOOGLE_API_KEY}" = "your_gemini_api_key_here" ]; then
        ...
        if ./mvnw -q test -Dgroups=evals -DexcludedGroups= 2>&1 | tail -40; then
