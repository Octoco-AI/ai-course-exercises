...
.DESCRIPTION
    ...
    prompt in src/main/java/ai/octoco/expensecategoriser/Core.java to bias the
    model toward "Other" for edge cases. That tanks the accuracy metric enough
    to fail the eval gate, so a PR from this branch is blocked by evals.yml
    until the prompt is fixed.
...
$target = 'src/main/java/ai/octoco/expensecategoriser/Core.java'
...
if ($regressed -eq $original) {
    Write-Error "Prompt block not found - has Core.java changed since the regression script was written?"
    exit 1
}

Set-Content -Path $target -Value $regressed -NoNewline
Write-Host "Regressed Core.java."
